...
//#region node_modules/react-router/dist/development/chunk-ZYFC6VSF.mjs
		...
		if (generateMiddlewareResponse) {
			...
			try {
				...
				let response = await runServerMiddlewarePipeline({
					...
					return await generateMiddlewareResponse(async (revalidationRequest) => {
	...
	async function queryRoute(request, { routeId, requestContext, dataStrategy, unstable_generateMiddlewareResponse: generateMiddlewareResponse } = {}) {
		...
		if (generateMiddlewareResponse) {
			...
			return await runServerMiddlewarePipeline({
				...
				if (isRouteErrorResponse(error)) return Promise.resolve(errorResponseToResponse(error));
...
function getMatchesToLoad(request, scopedContext, mapRouteProperties2, manifest, history, state, matches, submission, location, lazyRoutePropertiesToSkip, initialHydration, isRevalidationRequired, cancelledFetcherLoads, fetchersQueuedForDeletion, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, hasPatchRoutesOnNavigation, pendingActionResult) {
	...
	let dsMatches = matches.map((match, index) => {
		...
		return getDataStrategyMatch(mapRouteProperties2, manifest, request, match, lazyRoutePropertiesToSkip, scopedContext, shouldRevalidateLoader(match, shouldRevalidateArgs), shouldRevalidateArgs);
	});
...
function clientMiddlewareErrorHandler(error, routeId, matches, didCallHandler) {
	if (didCallHandler) return { [routeId]: {
		type: "error",
		result: error
	} };
	else return { [findNearestBoundary(matches, matches.find((m) => m.route.id === routeId || m.route.loader)?.route.id || routeId).route.id]: {
		type: "error",
		result: error
	} };
}
...
async function callServerRouteMiddleware(args, middlewares, handler, errorHandler, idx = 0) {
	...
	let next = async () => {
		...
		try {
			...
			if (isDataWithResponseInit(result)) result = dataWithResponseInitToResponse(result);
			nextResult = result;
			return nextResult;
		} catch (e) {
			nextResult = await errorHandler(isDataWithResponseInit(e) ? dataWithResponseInitToErrorResponse(e) : e, routeId);
			return nextResult;
		}
	};
	try {
		...
		if (isDataWithResponseInit(result)) result = dataWithResponseInitToResponse(result);
		if (nextCalled) return typeof result === "undefined" ? nextResult : result;
		else if (isResponse(result)) return result;
		else {
			nextResult = await next();
			return nextResult;
		}
	} catch (e) {
		return await errorHandler(isDataWithResponseInit(e) ? dataWithResponseInitToErrorResponse(e) : e, routeId);
	}
}
...
async function callClientRouteMiddleware(args, middlewares, handler, errorHandler, handlerResult = {}, idx = 0) {
	...
	try {
		...
		if (typeof result !== "undefined") console.warn("client middlewares are not intended to return values, the value will be ignored", result);
...
try {
	if (isBrowser) window.__reactRouterVersion = "7.8.0";
} catch (e) {}
...
//#endregion
//#region node_modules/react-router/dist/development/chunk-HZX6U7MI.mjs
/**
* react-router v7.8.0
*
* Copyright (c) Remix Software Inc.
...
function createRoutesStub(routes, _context) {
	return function RoutesTestStub({ initialEntries, initialIndex, hydrationData, future }) {
		...
		if (routerRef.current == null) {
			...
			routerRef.current = createMemoryRouter(processRoutes(convertRoutesToDataRoutes(routes, (r) => r), _context !== void 0 ? _context : future?.unstable_middleware ? new unstable_RouterContextProvider() : {}, frameworkContextRef.current.manifest, frameworkContextRef.current.routeModules), {
				initialEntries,
				initialIndex,
				hydrationData
			});
...
async function singleFetchAction(build, serverMode, staticHandler, request, handlerUrl, loadContext, handleError) {
	...
	function handleQueryResult(result) {
		if (!isResponse(result)) result = staticContextToResponse(result);
...
async function singleFetchLoaders(build, serverMode, staticHandler, request, handlerUrl, loadContext, handleError) {
	...
	function handleQueryResult(result) {
		...
		if (isRedirectResponse(response)) return generateSingleFetchResponse(request, build, serverMode, {
			result: { [SingleFetchRedirectSymbol]: getSingleFetchRedirect(response.status, response.headers, build.basename) },
...
function derive(build, mode) {
	...
	return {
		...
		dataRoutes,
		serverMode,
		staticHandler: createStaticHandler(dataRoutes, { basename: build.basename }),
		errorHandler: build.entry.module.handleError || ((error, { request }) => {
	...
	return async function requestHandler(request, initialContext) {
		...
		if (_build.future.unstable_middleware) {
			if (initialContext && !(initialContext instanceof unstable_RouterContextProvider)) {
				let error = /* @__PURE__ */ new Error("Invalid `context` value provided to `handleRequest`. When middleware is enabled you must return an instance of `unstable_RouterContextProvider` from your `getLoadContext` function.");
				handleError(error);
...
function RSCHydratedRouter({ createFromReadableStream, fetch: fetchImplementation = fetch, payload, routeDiscovery = "eager", unstable_getContext }) {
	if (payload.type !== "render") throw new Error("Invalid payload type");
	let router = import_react.useMemo(() => createRouterFromPayload({
		payload,
		fetchImplementation,
		unstable_getContext,
...
function HydratedRouter(props) {
	...
	} }, /* @__PURE__ */ import_react.createElement(RemixErrorBoundary, { location }, /* @__PURE__ */ import_react.createElement(RouterProvider2, { router }))), /* @__PURE__ */ import_react.createElement(import_react.Fragment, null));
}
//#endregion
